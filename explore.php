...
if (isset($_SESSION['user_id'])) {
    ...
    //Counts the amount of registered users the site has
    $userCount = $pdo->prepare("SELECT  COUNT(uid)  FROM users");
    $userCount->execute();
    $count = $userCount->fetchAll(PDO::FETCH_ASSOC);

    //Pulls out all users on yello
    $followerPull = $pdo->prepare("SELECT  *  FROM users");
...
<head>
    ...
    <meta charset="utf-8">
    <title><?=ucwords($user['username'])?>'s Explorer</title>
    ...
    <div class="row">
        ...
        <div class="col-lg-6 content">
            <div class="row">
                ...
                <div class="col-lg-11">
                    <h1 class="title" style="margin-bottom: 0px">Explore</h1>
                    <?php foreach ($count[0] as $userc) : ?>
                        <?php if($userc != null) { ?>
                            <p class="light-text"><?=$userc?> users</p>
                        <?php } else { ?>
                            <p class="light-text">No users</p>
                        <?php } endforeach; ?>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <?php foreach ($follower as $followeruser) : ?>
                        ...
                        foreach($following as $f) {
                            $result = $f['uid'];
                            if($result == $followeruser['uid']){
                                $userF =  $followeruser['uid'];
                            } else {
                        ...
                        <div class="row" style="padding-top: 15px">
                            ...
                            <div class="col-lg-3" style="padding-top: 7px">
                                ...
                                <?php } elseif($followeruser['uid'] == $userF) { ?>
