...
if (isset($_SESSION['user_id'])) {
    ...
    $followingPull = $pdo->prepare("SELECT f.follow_user, u.*  FROM follow AS f, users AS u WHERE f.follow_user = '$profile_uid' AND f.user_uid = u.uid;");
    $followingPull->execute();
    $followingusers = $followingPull->fetchAll(PDO::FETCH_ASSOC);
...
if(isset($_POST['unfollow'])){
    ...
    if($result) {
        header('Location: following.php?u=' . $profile_uid .'');
    }
}
?>
<!DOCTYPE html>
...
<div class="container">
    <div class="row">
        ...
        <div class="col-lg-6 content">
            ...
            <div class="row">
                <div class="col-lg-6">
                    <a href="followers.php?u=<?=$profile_uid?>" class="btn btn-block btn-theme-air">Followers</a>
                </div>
                <div class="col-lg-6">
                    <a href="following.php?u=<?=$profile_uid?>" class="btn btn-block btn-theme-air air-active">Following</a>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <?php foreach ($followingusers as $followinguser) : ?>
                        <div class="row" style="padding-top: 15px">
                            ...
                            <div class="col-lg-7" style="padding-left: 0px; padding-top: 7px">
                                <h3 class="tiny-title" style="margin-bottom: 0px"><?=$followinguser['display']?></h3>
                                <a href="profile.php?u=<?=$followinguser['uid']?>"><h4 class="yello-text" style="color: #f1c40f">@<?=$followinguser['username']?></h4></a>
                            </div>
                            <div class="col-lg-3" style="padding-top: 7px">
                                <?php if($followinguser['uid'] == $following['uid']) { ?>
                                    <form action="following.php?u=<?=$profile_uid?>" method="post">
                                        <input type="hidden" name="user" value="<?=$followinguser['uid']?>">
